...
def display_input_section(language: str):
    ...
    st.markdown(
        '<h3 class="input-title">💬 Ask Your Question / अपना प्रश्न पूछें</h3>',
        unsafe_allow_html=True,
    )
    ...
    if input_mode == "📝 Text Input":
        ...
        if audio_bytes:
            st.markdown(
                '<div class="success-message">✅ Audio recorded successfully! Ready to process...</div>',
                unsafe_allow_html=True,
            )
            input_type = "audio"
            ...
            if audio_size < 1024:
                st.markdown(
                    '<div class="error-message">⚠️ Audio seems very short. Please record a longer message for better results.</div>',
                    unsafe_allow_html=True,
                )
        else:
            st.info(
                "🎯 No audio recorded yet. Click the microphone button above to start recording."
            )
            input_type = "text"
            input_data = ""
...
def display_response(response, language: str):
    ...
    if not response.success:
        st.markdown(
            f'<div class="error-message">❌ {response.text_response}</div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        '<h3 style="color: white; text-align: center; margin-bottom: 1rem;">🎉 Response Ready!</h3>',
        unsafe_allow_html=True,
    )

    # Display text response with nice formatting
    st.markdown(
        f'<div class="response-text">{response.text_response}</div>',
        unsafe_allow_html=True,
    )
    ...
    if response.audio_response:
        st.markdown(
            '<h4 style="color: white;">🔊 Listen to Response:</h4>',
            unsafe_allow_html=True,
        )
    ...
    if response.technical_terms:
        st.markdown(
            '<h4 style="color: white; margin-top: 2rem;">📚 Technical Terms / तकनीकी शब्द</h4>',
            unsafe_allow_html=True,
        )

        for i, term in enumerate(response.technical_terms):
            with st.expander(
                f"🔍 {term.original} → {term.translation}", expanded=i == 0
            ):
...
def main():
    ...
    if submit_button:
        if not input_data:
            st.markdown(
                '<div class="error-message">⚠️ Please enter a question first!</div>',
                unsafe_allow_html=True,
            )
        else:
                    ...
                    if isinstance(input_data, bytes):
                        ...
                        if response.success and hasattr(response, "text_response"):
                            st.session_state.query_history.append(
                                f"[Voice] {response.text_response[:50]}..."
                            )
                    else:
